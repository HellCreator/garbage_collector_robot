	...
	switch(ADC_channel)
	{
		case POMIAR_M1:
		{
			...
			if(--ile_pomiarow == 0)
			{
				...
				//pomiar tylko na ADC3
				ADMUX = (1<<REFS0)| 0b00011 ;
				
                uint_global_prad_M1 = MOTOR_I_SENS_MUL*(pomiar >> 4);//wyliczenie sredniej, przesuniecie w prawo o 4 bity jest rownowazne dzieleniu przez 16
		...
		case POMIAR_M2:
		{
			...
			if(--ile_pomiarow == 0)
			{
                ...
                uint_global_prad_M2 = MOTOR_I_SENS_MUL*(pomiar >> 4);//wyliczenie sredniej, przesuniecie w prawo o 4 bity jest rownowazne dzieleniu przez 16
                										//wynik wyskalowany tak zeby byl w mA					
				pomiar = 0;
		...
		case POMIAR_ADC6:
		{
			...
			{
				ADC_turn_off(); //ma na celu upewnienie sie zenastepny wynik nie bedzi dotyczyl poprzedniego kanalu ADC
				ADC_channel = POMIAR_ADC7;
				ile_pomiarow = ADC_ILOSC_POMIAROW;
				
				//wejscie roznicowe pomiaru pradu M1 (ADC1-ADC0), wzmocnienie 10
				ADMUX = (1<<REFS0) | 0b00111 ;				
				
				uint_global_adc6 = pomiar >> 4;//wyliczenie sredniej, przesuniecie w prawo o 4 bity jest rownowazne dzieleniu przez 16
                										//wynik wyskalowany tak zeby byl w mA
				pomiar=0;
        ...
        case POMIAR_ADC7:
		{
			...
			if(--ile_pomiarow == 0)
			{
				...
				//Pomiar tylko na ADC1
				ADMUX = (1<<REFS0) | 0b00001 ;
				
				uint_global_adc7 = pomiar >> 4;//wyliczenie sredniej, przesuniecie w prawo o 4 bity jest rownowazne dzieleniu przez 16
                										//wynik wyskalowany tak zeby byl w mA
				pomiar=0;
