...
def run():
    ...
    imgsz=[448,288]  # inference size (pixels)
    ...
    while(True):
        ...
        # Apply padding on two sides
        padding_left = 50
        padding_right = 50
        img_padded = np.pad(img, ((0, 0), (padding_left, padding_right), (0, 0)), mode='constant', constant_values=0)   
        ...
        #img = img[8:232,4:420]
        img0 = img.copy()
        img_padded = img[np.newaxis, :, :, :]        
        # Stack
        img_padded = np.pad(img_padded, pad_width=48)
        img_padded = np.stack(img_padded, 0)

        # Convert
        img_padded = img_padded[..., ::-1].transpose((0, 3, 1, 2))  # BGR to RGB, BHWC to BCHW
        img_padded = np.ascontiguousarray(img_padded)
        
        img_padded = torch.from_numpy(img_padded).to(device)
        img_padded = img_padded.half() if half else img_padded.float()  # uint8 to fp16/32
        ...
        # Inference
        t1 = time_sync()
        pred = model(img_padded, augment=augment,
        ...
        # Apply Classifier
        if classify:
            pred = apply_classifier(pred, modelc, img_padded, img0)

        # Process detections
        for i, det in enumerate(pred):  # detections per image
            s = f'{i}: '
            s += '%gx%g ' % img_padded.shape[2:]  # print string
            annotator = Annotator(img0, line_width=line_thickness, example=str(names))
            annotator1 = Annotator(depth_colormap, line_width=line_thickness, example=str(names))
            if len(det):
                # Rescale boxes from img_size to im0 size
                det[:, :4] = scale_coords(img_padded.shape[2:], det[:, :4], img0.shape).round()
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if x != [] or y != []:
                        ...
                        if (x >= 290):
                            theta = (((x-290)/(424-290))*(hfov*((424-290)/290)))
                        else:
                            theta = (((x-290)/(290))*(hfov*((290)/424)))
                        lines.append(theta)
